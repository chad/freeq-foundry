...
export class FreeqClient extends EventEmitter {
    ...
    _seenCoordinationEvents = new Map();
    constructor(opts) {
        super();
        this.opts = opts;
            ...
            encryptFn().then((encrypted) => {
                ...
                if (encrypted.length + 200 <= perChunkBudget || !multilineCap) {
                    ...
                    if (chunks.length > this.multilineMaxLines) {
                        this.emit('systemMessage', wireTarget, `Message too large to send: ciphertext exceeds server multiline limit (${this.multilineMaxLines} lines).`);
                        return;
                    }
    ...
    requestHistory(channelOrOpts, before) {
        const count = 50;
        ...
        // older servers.
        const targetOf = (x) => typeof x === 'string' ? x : x.target;
        ...
        switch (opts.mode) {
            ...
            case 'before':
                if (!marker)
                    throw new Error("requestHistory mode='before' requires opts.msgid or opts.timestamp");
                this.raw(`CHATHISTORY BEFORE ${opts.target} ${marker} ${c}`);
                break;
            case 'after':
                if (!marker)
                    throw new Error("requestHistory mode='after' requires opts.msgid or opts.timestamp");
    ...
    requestHistoryTargets(limit = 50) {
    ...
    onTransportStateChange(state) {
        ...
        if (state === 'connected') {
            ...
            const sendRegistration = (token) => {
                ...
                if (registrationSent || !this.transport)
                    return;
    ...
    isSelfSender(from, tags) {
        const acct = tags?.['+freeq.at/account'] ?? tags?.['account'];
        if (acct && this._authDid && acct === this._authDid)
            return true;
     ...
     */
    rememberSenderDid(from, tags) {
        const did = tags?.['+freeq.at/account'] ?? tags?.['account'];
        if (!did || !isDid(did) || !from || from.startsWith('#') || from.startsWith('&'))
            return;
     ...
     *  the event only once. */
    emitCoordinationEvent(channel, from, tags) {
        ...
        if (eventId) {
            ...
            if (seen !== undefined && now - seen < 30_000)
                return; // dup
            ...
            if (this._seenCoordinationEvents.size > 1000) {
                ...
                for (const [k, t] of this._seenCoordinationEvents) {
                    if (t < cutoff)
                        this._seenCoordinationEvents.delete(k);
                }
            }
        }
    ...
    async dispatchAssembledMultiline(batch) {
        ...
        if (!isChannel && !isSelf)
            this.rememberSenderDid(from, openerTags);
        ...
        if (openerTags['+draft/edit']) {
            ...
            this.emit('messageEdited', bufName, openerTags['+draft/edit'], displayText, openerTags['msgid'], isStreaming, from, openerTags['account']);
            return;
        }
    ...
    groupChunksIntoBatches(chunks) {
        ...
        for (const c of chunks) {
            ...
            const startsNewBatch = cur.length > 0 &&
                !c.concat &&
    ...
    async handleLine(rawLine) {
        ...
        switch (msg.command) {
            ...
            case 'PRIVMSG': {
                ...
                if (!isChannel && !isSelf)
                    this.rememberSenderDid(from, msg.tags);
                // DM thread key = the peer's canonical DID when known (else the nick):
                    ...
                    if (editBatch && editBatch.type !== 'draft/multiline') {
                        ...
                        if (reactionsTag) {
                            ...
                            for (const part of reactionsTag.split(';')) {
                                const [emoji, nicks] = part.split(':');
                                if (emoji && nicks) {
                                    const set = editReactions.get(emoji) ?? new Set();
                                    for (const n of nicks.split(','))
                                        if (n)
                                            set.add(n);
                        ...
                        const idx = editBatch.messages.findIndex((m) => m.id === editOf || m.editOf === editOf);
                        if (idx >= 0) {
            ...
            case 'TAGMSG': {
                ...
                if (deleteOf) {
                    this.emit('messageDeleted', bufName, deleteOf, from, msg.tags['account']);
                    break;
                }
                ...
                if (govSignal) {
                    const validSignals = ['pause', 'resume', 'revoke', 'approval_granted', 'approval_denied', 'budget_exceeded'];
                    if (validSignals.includes(govSignal)) {
                ...
                if (avError) {
                    this.emit('avError', avError, avId || '', msg.tags['+freeq.at/av-reason'] || '');
                }
                break;
            }
            case 'TOPIC': {
            ...
            case 'MARKREAD': {
                ...
                if (target) {
                    ...
                    const timestamp = raw && raw !== '*' && raw.startsWith('timestamp=')
            ...
            case '401': {
                ...
                const shown = failTarget && isDid(failTarget)
                    ...
                    : failTarget;
                this.emit('systemMessage', failTarget ? this.dmKey(failTarget) : 'server', `${shown} is offline — message saved, they'll see it next time they connect`);
                break;
            }
            case '404':
            ...
            case '330': {
                ...
                if (whoisNick && did) {
                    ...
                    if (prevDid && prevDid !== did)
                        this._didToNick.delete(prevDid);
                    const prevNick = this._didToNick.get(did);
                    if (prevNick && prevNick !== lc)
                        this._nickToDid.delete(prevNick);
                    this._nickToDid.set(lc, did);
                    this._didToNick.set(did, lc);
    ...
    sendMedia(target, media) {
        const tags = { '+freeq.at/media-url': media.url };
        if (media.mime)
            tags['+freeq.at/media-mime'] = media.mime;
        if (media.alt)
            tags['+freeq.at/media-alt'] = media.alt;
        if (media.width !== undefined)
            tags['+freeq.at/media-w'] = String(media.width);
        if (media.height !== undefined)
            tags['+freeq.at/media-h'] = String(media.height);
        if (media.durationMs !== undefined)
            tags['+freeq.at/media-duration'] = String(media.durationMs);
        if (media.sizeBytes !== undefined)
            tags['+freeq.at/media-size'] = String(media.sizeBytes);
        const body = media.fallback ?? `📎 ${media.url}`;
        this.raw(format('PRIVMSG', [target, body], tags));
    }
    /** Attach link-preview metadata to a message. */
    sendLinkPreview(target, preview) {
        const tags = { '+freeq.at/link-url': preview.url };
        if (preview.title)
            tags['+freeq.at/link-title'] = preview.title;
        if (preview.description)
            tags['+freeq.at/link-desc'] = preview.description;
        if (preview.imageUrl)
            tags['+freeq.at/link-image'] = preview.imageUrl;
    ...
    sendAndAwaitEcho(target, text, tags = {}) {
        return new Promise((resolve, reject) => {
            ...
            const onRaw = (_line, parsed) => {
                if (parsed.command !== 'PRIVMSG' && parsed.command !== 'TAGMSG')
                    return;
                if (parsed.tags?.['+freeq.at/echo-nonce'] !== nonce)
                    return;
     ...
     *  `reply_in_thread`). */
    sendReplyInThread(target, parentMsgId, text) {
    ...
    sendHeartbeat(state, ttlSeconds) {
    ...
    requestApproval(channel, capability, resource) {
    ...
    emitEvent(channel, eventType, payload, opts = {}) {
        ...
        if (opts.refId)
            tags['+freeq.at/task-id'] = opts.refId;
        if (opts.extraTags)
    ...
    createTask(channel, description) {
    ...
    updateTask(channel, taskId, phase, summary) {
    ...
    completeTask(channel, taskId, summary, url) {
    ...
    attachEvidence(channel, taskId, evidenceType, summary, url) {
        const payload = { type: evidenceType, summary };
        if (url)
            payload.url = url;
        const urlStr = url ? ` — ${url}` : '';
    ...
    sendAsChild(childNick, channel, text) {
    ...
    submitSpend(channel, amount, unit, description, taskRef) {
    ...
    setBudget(channel, maxAmount, unit, period, sponsorDid) {
